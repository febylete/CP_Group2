...
To test this hypothesis, 3 separate t-tests will be used to compare -- between the participants in the meme and news-only feeds -- the scores of each of these three measurements.Regression will also be performed to include other co-variates that the client believes may affect participants' responses.

### H1 Assumption checking

1 & 2) Independence and Randomness
Since participants were randomly assigned to the conditions and only have a single observation, we know that assumptions of randomness and independence are met.
...
This hypothesis was tested with ANOVAs/ANCOVAs. However, the client requested that the assumptions for these tests be repeated in order to make sure that these tests were appropriate.

## H2 Assumption checking
The first assumption to be checked was the normality of residuals.This assumption was checked for the Chinese and English datasets individually, since the client ran the tests on each sample separately; the assumption was also checked for the combined dataset, since it is seemed appropriate to check given that the data for the two groups are capable of being combined and the pertinent hypothesis does not explicitly reference a distinction between the two.
