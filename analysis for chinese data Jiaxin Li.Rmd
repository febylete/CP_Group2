...
```{r}
library(car)
leveneTest(Credibility_Total ~ feed_type, data = chinese_data_filtered)
```
...
```{r}
library(car)
leveneTest(newsworth_all ~ feed_type, data = chinese_data_filtered)
...
```{r}
library(car)
ancova_model_credibility <- aov(Credibility_Total ~ feed_type + NewsSharing_SM + NewsCred_SM, data = chinese_data_filtered)
...
```{r}
library(car)
ancova_model_newsworthiness <- aov(newsworth_all ~ feed_type + NewsSharing_SM + NewsCred_SM, data = chinese_data_filtered)
