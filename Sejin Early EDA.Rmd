...
## Over 30 Seconds
```{r}
Data$ThirtySPlus <- ifelse(Data$Durationinseconds>3600,1,0)

CN_Over30 <- Data |> 
  filter(Group==1) |> 
  mutate(ThirtySPlus = ifelse(Durationinseconds>3600,1,0))
EN_Over30 <- Data |> 
  filter(Group==0) |> 
  mutate(ThirtySPlus = ifelse(Durationinseconds>3600,1,0))
...
ggplot(Data,aes(Group,Durationinseconds))+geom_point(aes(color=ThirtySPlus))+geom_hline(yintercept = 3600)
